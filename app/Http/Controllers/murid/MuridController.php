...
class MuridController extends Controller
{
	public function murid()
	{
		...
		return view('murid/murid', compact('murid','kelas'));
	}
	public function viewmurid(Request $request)
	{
		$id = $request->id;		
		$murid = Murid::where('kelas', $id)->orderBy('created_at', 'asc')->get();
		return view('murid/viewmurid', compact('murid'));
	}
	...
	public function simpanmurid(Request $request)
	{
		$validator = Validator::make($request->all(), [
			...
			"status_murid.required" => "Status tidak boleh kosong"
		...
		if ($request->hasFile('file_suratpenerimaan')) {
		...
		if ($request->hasFile('file_suratkenaikankelas')) {
		...
		if ($request->hasFile('file_suratkelulusan')) {
